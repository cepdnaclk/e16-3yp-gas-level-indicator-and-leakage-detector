...
Router.post('/',function(req,res){     
    ...
    connection.query("select device_id from devices where device_id = '"+device_id+"'",function (err, data) { //check if user already exists
        ...
        if (!Object.keys(data).length) return res.sendStatus(400) //device doesn't exists
  
        connection.query('SELECT leakage FROM devices WHERE device_id = ?', [req.body.device_id], (err, rows, fields) => {
